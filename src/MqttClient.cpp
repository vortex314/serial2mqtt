...
int MqttClient::onMessage(void *context, char *topicName, int topicLen, MQTTAsync_message *message)
{
    Bytes bytes((uint8_t*)message->payload,message->payloadlen);
    Cbor cbor(1000);
    cbor.addKeyValue(H("topic"),topicName).addKeyValue(H("message"),bytes).addKeyValue(H("qos"),message->qos).addKeyValue(H("retained"),message->retained);
...
void MqttClient::onDisconnect(void* context, MQTTAsync_successData* response)
{
    ...
    disc_finished = 1;
    eb.publish(H("mqtt.disconnected"));
}
...
Erc MqttClient::connect(Str& connection,Str& willTopic,Str& willMessage,int willQos)
{
...
//        MQTTAsync_token token;
    ...
    MQTTAsync_create(&client, connection.c_str(), CLIENTID, MQTTCLIENT_PERSISTENCE_NONE, NULL);
...
{
    MQTTAsync_responseOptions opts = MQTTAsync_responseOptions_initializer;
    LOGF("Subscribing to topic %s for client %s using QoS%d", topic.c_str(), CLIENTID, qos);
